...
__global__ void setWaveField(float*Vx,float*Vy,float*Vz,float*Txx,float*Tyy,float*Tzz,float*Txz,float*Tyz,float*Txy,int n)
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;

    Vx[index] = 0.0f;   Vy[index] = 0.0f;  Vz[index] = 0.0f;
    Txx[index] = 0.0f; Tyy[index] = 0.0f; Tzz[index] = 0.0f;
    Txz[index] = 0.0f; Tyz[index] = 0.0f; Tyz[index] = 0.0f;
}

__global__ void computeStress(float*Vx,float*Vy,float*Vz,float*Txx,float*Tyy,float*Tzz,float*Txy,float*Txz,float*Tyz,float*M,float*L,float*source,int*xsrc,int*ysrc,int*zsrc,int nsrc,int timePointer,int shotPointer,int nx,int ny,int nz,float dx,float dy,float dz,float dt) 
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;

    int kk = index/(nx*ny);          // indicador de matrizes (direção z)
    int jj = index % nx;             // indicador de colunas  (direção x)
    int ii = (index % (nx*ny)) / nx; // indicador de linhas   (direção y)  
...
__global__ void computeVelocity(float*Vx,float*Vy,float*Vz,float*Txx,float*Tyy,float*Tzz,float*Txy,float*Txz,float*Tyz,float*rho,int nx,int ny,int nz,float dx,float dy,float dz,float dt) 
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void cerjanElasticABC3D(float*Vx,float*Vy,float*Vz,float*Txx,float*Tyy,float*Tzz,float*Txy,float*Txz,float*Tyz,float*cubXYZ,int n)
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void getPressureWaveField(float*Txx,float*Tyy,float*Tzz,float*Pss,int nPoints)
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void getPWaveField(float*Ux,float*Uy,float*Uz,float*P,int nx,int ny,int nz,float dx,float dy,float dz)
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void getSWaveField(float*Ux,float*Uy,float*Uz,float*Shx,float*Shy,float*Sv,int nx,int ny,int nz,float dx,float dy,float dz)
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void getSeismogram(float*seism,float*field,int*xrec,int*yrec,int*zrec,int nrecx,int nrecy,int nrecs,int nt,int nxx,int nyy,int nzz,int timePointer)
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;

    int recx = index % nrecx;                          
    int recy = (index % (nrecx*nrecy)) / nrecx;   
